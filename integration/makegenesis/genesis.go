...
func MakeGenesisStore() *genesisstore.Store {
	...
	totalSupply := futils.ToRlv(uint64(_total) * 1e6)
	balance := futils.ToRlv(uint64(_validator) * 1e6)
	stake := futils.ToRlv(uint64(_staker) * 1e6)
	initialBalance := futils.ToRlv(uint64(_initial) * 1e6)
